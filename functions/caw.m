...
% 
% Inputs:
%     Xini : F*T*J initial STFTs obtained e.g. with anisotropic Wiener filtering
...
function [Xe,iter,score] = caw(Xini,variances,kappa,delta,Nw,hop,wtype,tol_pcg,max_iter,sm)
...
for j=1:J-1
    ...
    mu_single(:,:,1) = angle(Xini(:,:,j));
    mu_single(:,:,2) = angle(sum(Xini(:,:,j+1:end),3));
